...
Define_Module(Ieee80211MacWithRA);

// don't forget to keep synchronized the C++ enum and the runtime enum definition
Register_Enum(Ieee80211MacWithRA,
              (Ieee80211MacWithRA::IDLE,
               Ieee80211MacWithRA::DEFER,
               Ieee80211MacWithRA::WAITAIFS,
               Ieee80211MacWithRA::BACKOFF,
               Ieee80211MacWithRA::WAITACK,
               Ieee80211MacWithRA::WAITMULTICAST,
               Ieee80211MacWithRA::WAITCTS,
               Ieee80211MacWithRA::WAITSIFS,
               Ieee80211MacWithRA::RECEIVE));
...
Ieee80211MacWithRA::Ieee80211MacWithRA()
{
    endSIFS = NULL;
    endDIFS = NULL;
    endTimeout = NULL;
 ...
 */
void Ieee80211MacWithRA::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        for (int i=0; i<numCategories(); i++)
        {
            ...
            {
                transmissionQueue(i)->createClassifier("Ieee80211MacWithRAQueueClassifier");
                if (numCategories()==1)
            ...
            {
                transmissionQueue(i)->createClassifier("Ieee80211MacWithRAQueueClassifier2");
                if (numCategories()==1)
                    transmissionQueue(i)->setMaxSize(maxQueueSize);
                else
                    transmissionQueue(i)->setMaxSize(maxCategorieQueueSize);
        ...
        // state variables
        fsm.setName("Ieee80211MacWithRA State Machine");
        mode = DCF;
        sequenceNumber = 0;
        ...
        stateVector.setEnum("Ieee80211MacWithRA");
...
void Ieee80211MacWithRA::initWatches()
{
// initialize watches
     WATCH(fsm);
     WATCH(radioState);
...
void Ieee80211MacWithRA::configureAutoBitRate()
{
    ...
    case 3:
    {
        ...
        //Set the timer to recalculate the bit-rate
        ...
        //** end configure timer
        //Create CARS object to getting PER with rate adaptation
        carsPER = new CARS();
        //Distance of the clients and AccessPoint
        Coord apCoord;
        ...
        Coord cliCoord = MobilityAccess().get(module_cli)->getCurrentPosition();
        previous_distance = getDistance(cliCoord,apCoord);


        EV<<"MAC Transmission algorithm : Context-Aware Rate Selection algorithm" << endl;
        break;
    }
    default:
    {
...
void Ieee80211MacWithRA::finish()
{
...
void Ieee80211MacWithRA::registerInterface()
{
...
void Ieee80211MacWithRA::initializeQueueModule()
{
 ...
 */
void Ieee80211MacWithRA::handleSelfMsg(cMessage *msg)
{
    ...
    if (strcmp(msg->getName(),"autobitrateTimer") == 0)
    {
        ...
        speed = par("speed"); //TODO this parameter must be dynamic with real-time information of the Context application, using linearMobilityWithWrap for the application
        packet_length = par("packetLength");//TODO
    ...
    if ((strcmp(msg->getName(), "Backoff") == 0) || (strcmp(msg->getName(), "AIFS")==0))
    {
        ...
        for (unsigned int i = numCategories()-1; (int)i > kind; i--)  //
        {
...
void Ieee80211MacWithRA::handleUpperMsg(cPacket *msg)
{
...
#ifdef  USEMULTIQUEUE
int Ieee80211MacWithRA::mappingAccessCategory(Ieee80211DataOrMgmtFrame *frame)
{
...
void Ieee80211MacWithRA::handleCommand(cMessage *msg)
{
...
void Ieee80211MacWithRA::handleLowerMsg(cPacket *msg)
{
...
void Ieee80211MacWithRA::receiveChangeNotification(int category, const cObject *details)
{
    Enter_Method_Silent();
 ...
 */
void Ieee80211MacWithRA::handleWithFSM(cMessage *msg)
{
...
void Ieee80211MacWithRA::finishReception()
{
    if (getCurrentTransmission())
    {
        backoff() = true;
 ...
 * Timing functions.
 */
simtime_t Ieee80211MacWithRA::getSIFS()
{
...
simtime_t Ieee80211MacWithRA::getSlotTime()
{
...
simtime_t Ieee80211MacWithRA::getPIFS()
{
    return getSIFS() + getSlotTime();
}

simtime_t Ieee80211MacWithRA::getDIFS(int category)
{
...
simtime_t Ieee80211MacWithRA::getHeaderTime(double bitrate)
{
    ModulationType modType;
    modType = WifiModulationType::getModulationType(opMode, bitrate);
...
simtime_t Ieee80211MacWithRA::getAIFS(int AccessCategory)
{
    return AIFSN(AccessCategory) * getSlotTime() + getSIFS();
}

simtime_t Ieee80211MacWithRA::getEIFS()
{
// FIXME:   return getSIFS() + getDIFS() + (8 * ACKSize + aPreambleLength + aPLCPHeaderLength) / lowestDatarate;
    ...
    else
    {
        ...
        // what is the real unit? The use seems to be inconsistent between b and g modes.
        if (opMode=='b')
...
simtime_t Ieee80211MacWithRA::computeBackoffPeriod(Ieee80211Frame *msg, int r)
{
    int cw;
 ...
 * Timer functions.
 */
void Ieee80211MacWithRA::scheduleSIFSPeriod(Ieee80211Frame *frame)
{
...
void Ieee80211MacWithRA::scheduleDIFSPeriod()
{
...
void Ieee80211MacWithRA::cancelDIFSPeriod()
{
    EV << "canceling DIFS period\n";
    cancelEvent(endDIFS);
}

void Ieee80211MacWithRA::scheduleAIFSPeriod()
{
    bool schedule = false;
    for (int i = 0; i<numCategories(); i++)
    {
...
void Ieee80211MacWithRA::rescheduleAIFSPeriod(int AccessCategory)
{
    ASSERT(1);
...
void Ieee80211MacWithRA::cancelAIFSPeriod()
{
...
//XXXvoid Ieee80211MacWithRA::checkInternalColision()
//{
//  EV << "We obtain endAIFS, so we have to check if there
//}


void Ieee80211MacWithRA::scheduleDataTimeoutPeriod(Ieee80211DataOrMgmtFrame *frameToSend)
{
    double tim;
...
void Ieee80211MacWithRA::scheduleMulticastTimeoutPeriod(Ieee80211DataOrMgmtFrame *frameToSend)
{
    if (!endTimeout->isScheduled())
    {
...
void Ieee80211MacWithRA::cancelTimeoutPeriod()
{
...
void Ieee80211MacWithRA::scheduleCTSTimeoutPeriod()
{
    if (!endTimeout->isScheduled())
    {
...
void Ieee80211MacWithRA::scheduleReservePeriod(Ieee80211Frame *frame)
{
...
void Ieee80211MacWithRA::invalidateBackoffPeriod()
{
    backoffPeriod() = -1;
}

bool Ieee80211MacWithRA::isInvalidBackoffPeriod()
{
    return backoffPeriod() == -1;
}

void Ieee80211MacWithRA::generateBackoffPeriod()
{
...
void Ieee80211MacWithRA::decreaseBackoffPeriod()
{
    // see spec 9.9.1.5
...
void Ieee80211MacWithRA::scheduleBackoffPeriod()
{
    EV << "scheduling backoff period\n";
    scheduleAt(simTime() + backoffPeriod(), endBackoff());
}

void Ieee80211MacWithRA::cancelBackoffPeriod()
{
 ...
 */
void Ieee80211MacWithRA::sendACKFrameOnEndSIFS()
{
...
void Ieee80211MacWithRA::sendACKFrame(Ieee80211DataOrMgmtFrame *frameToACK)
{
...
void Ieee80211MacWithRA::sendDataFrameOnEndSIFS(Ieee80211DataOrMgmtFrame *frameToSend)
{
...
#ifdef  USEMULTIQUEUE
void Ieee80211MacWithRA::sendDataFrame(Ieee80211DataOrMgmtFrame *frameToSend)
{
...
void Ieee80211MacWithRA::sendRTSFrame(Ieee80211DataOrMgmtFrame *frameToSend)
{
    EV << "sending RTS frame\n";
    sendDown(setBasicBitrate(buildRTSFrame(frameToSend)));
}

void Ieee80211MacWithRA::sendMulticastFrame(Ieee80211DataOrMgmtFrame *frameToSend)
{
...
void Ieee80211MacWithRA::sendCTSFrameOnEndSIFS()
{
...
void Ieee80211MacWithRA::sendCTSFrame(Ieee80211RTSFrame *rtsFrame)
{
 ...
 */
Ieee80211DataOrMgmtFrame *Ieee80211MacWithRA::buildDataFrame(Ieee80211DataOrMgmtFrame *frameToSend)
{
...
Ieee80211ACKFrame *Ieee80211MacWithRA::buildACKFrame(Ieee80211DataOrMgmtFrame *frameToACK)
{
...
Ieee80211RTSFrame *Ieee80211MacWithRA::buildRTSFrame(Ieee80211DataOrMgmtFrame *frameToSend)
{
...
Ieee80211CTSFrame *Ieee80211MacWithRA::buildCTSFrame(Ieee80211RTSFrame *rtsFrame)
{
...
Ieee80211DataOrMgmtFrame *Ieee80211MacWithRA::buildMulticastFrame(Ieee80211DataOrMgmtFrame *frameToSend)
{
    Ieee80211DataOrMgmtFrame *frame = (Ieee80211DataOrMgmtFrame *)frameToSend->dup();
...
Ieee80211Frame *Ieee80211MacWithRA::setBasicBitrate(Ieee80211Frame *frame)
{
...
Ieee80211Frame *Ieee80211MacWithRA::setBitrateFrame(Ieee80211Frame *frame)
{
 ...
 * Helper functions.
 */
void Ieee80211MacWithRA::finishCurrentTransmission()
{
    popTransmissionQueue();
    resetStateVariables();
}

void Ieee80211MacWithRA::giveUpCurrentTransmission()
{
...
void Ieee80211MacWithRA::retryCurrentTransmission()
{
...
Ieee80211DataOrMgmtFrame *Ieee80211MacWithRA::getCurrentTransmission()
{
    return transmissionQueue()->empty() ? NULL : (Ieee80211DataOrMgmtFrame *)transmissionQueue()->front();
}

void Ieee80211MacWithRA::sendDownPendingRadioConfigMsg()
{
    if (pendingRadioConfigMsg != NULL)
    {
...
void Ieee80211MacWithRA::setMode(Mode mode)
{
    if (mode == PCF)
...
void Ieee80211MacWithRA::resetStateVariables()
{
    backoffPeriod() = 0;
    if (rateControlMode == RATE_AARF || rateControlMode == RATE_ARF)
        reportDataOk();
...
bool Ieee80211MacWithRA::isMediumStateChange(cMessage *msg)
{
    return msg == mediumStateChange || (msg == endReserve && radioState == RadioState::IDLE);
}

bool Ieee80211MacWithRA::isMediumFree()
{
    return radioState == RadioState::IDLE && !endReserve->isScheduled();
}

bool Ieee80211MacWithRA::isMulticast(Ieee80211Frame *frame)
{
    return frame && frame->getReceiverAddress().isMulticast();
}

bool Ieee80211MacWithRA::isForUs(Ieee80211Frame *frame)
{
    return frame && frame->getReceiverAddress() == address;
}

bool Ieee80211MacWithRA::isSentByUs(Ieee80211Frame *frame)
{
...
bool Ieee80211MacWithRA::isDataOrMgmtFrame(Ieee80211Frame *frame)
{
    return dynamic_cast<Ieee80211DataOrMgmtFrame*>(frame);
}

bool Ieee80211MacWithRA::isMsgAIFS(cMessage *msg)
{
    for (int i = 0; i<numCategories(); i++)
        if (msg == endAIFS(i))
            return true;
...
Ieee80211Frame *Ieee80211MacWithRA::getFrameReceivedBeforeSIFS()
{
    return (Ieee80211Frame *)endSIFS->getContextPointer();
}

void Ieee80211MacWithRA::popTransmissionQueue()
{
...
double Ieee80211MacWithRA::computeFrameDuration(Ieee80211Frame *msg)
{
...
double Ieee80211MacWithRA::computeFrameDuration(int bits, double bitrate)
{
    double duration;
    if (PHY_HEADER_LENGTH<0)
    {
    ...
    {
        // FIXME: check how PHY_HEADER_LENGTH is handled. Is that given in bytes or secs ???
        // what is the real unit? The use seems to be inconsistent betwen b and g/a/p modes.
...
void Ieee80211MacWithRA::logState()
{
    std::string a[10];
    std::string b[10];
...
const char *Ieee80211MacWithRA::modeName(int mode)
{
...
bool Ieee80211MacWithRA::transmissionQueueEmpty()
{
    for (int i=0; i<numCategories(); i++)
...
unsigned int Ieee80211MacWithRA::transmissionQueueSize()
{
...
void Ieee80211MacWithRA::reportDataOk()
{
    retryCounter() = 0;
    if (rateControlMode==RATE_CR)
       return;
...
void Ieee80211MacWithRA::reportDataFailed(void)
{
    retryCounter()++;
    if (rateControlMode==RATE_CR)
       return;
...
int Ieee80211MacWithRA::getMinTimerTimeout(void)
{
    return minTimerTimeout;
}

int Ieee80211MacWithRA::getMinSuccessThreshold(void)
{
    return minSuccessThreshold;
}

int Ieee80211MacWithRA::getTimerTimeout(void)
{
    return timerTimeout;
}

int Ieee80211MacWithRA::getSuccessThreshold(void)
{
    return successThreshold;
}

void Ieee80211MacWithRA::setTimerTimeout(int timer_timeout)
{
...
void Ieee80211MacWithRA::setSuccessThreshold(int success_threshold)
{
...
void Ieee80211MacWithRA::reportRecoveryFailure(void)
{
...
void Ieee80211MacWithRA::reportFailure(void)
{
    if (rateControlMode == RATE_AARF)
    {
...
bool Ieee80211MacWithRA::needRecoveryFallback(void)
{
    if (retryCounter() == 1)
    {
        return true;
...
bool Ieee80211MacWithRA::needNormalFallback(void)
{
...
double Ieee80211MacWithRA::getBitrate()
{
    return bitrate;
}

void Ieee80211MacWithRA::setBitrate(double rate)
{
    bitrate = rate;
}
...
bool & Ieee80211MacWithRA::backoff(int i)
{
    if (i==-1)
         i = currentAC;
...
simtime_t & Ieee80211MacWithRA::TXOP(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
simtime_t & Ieee80211MacWithRA::backoffPeriod(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
int & Ieee80211MacWithRA::retryCounter(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
int & Ieee80211MacWithRA::AIFSN(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
int & Ieee80211MacWithRA::cwMax(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
int & Ieee80211MacWithRA::cwMin(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
cMessage * Ieee80211MacWithRA::endAIFS(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
cMessage * Ieee80211MacWithRA::endBackoff(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
const bool Ieee80211MacWithRA::isBakoffMsg(cMessage *msg)
{
    for (unsigned int i=0; i<edcCAF.size(); i++)
    {
...
// Statistics
long & Ieee80211MacWithRA::numRetry(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
long & Ieee80211MacWithRA::numSentWithoutRetry(int i)
{
    if (i==-1)
         i = currentAC;
...
long & Ieee80211MacWithRA::numGivenUp(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
long & Ieee80211MacWithRA::numSent(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
long & Ieee80211MacWithRA::numDropped(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
long & Ieee80211MacWithRA::bites(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
simtime_t & Ieee80211MacWithRA::minjitter(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
simtime_t & Ieee80211MacWithRA::maxjitter(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
cOutVector * Ieee80211MacWithRA::jitter(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
cOutVector * Ieee80211MacWithRA::macDelay(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
cOutVector * Ieee80211MacWithRA::throughput(int i)
{
    if (i==-1)
         i = currentAC;
    if (i>=(int)edcCAF.size())
...
Ieee80211MacWithRA::Ieee80211DataOrMgmtFrameList * Ieee80211MacWithRA::transmissionQueue(int i)
{
    if (i==-1)
        i = currentAC;
    if (i>=(int)edcCAF.size())
...
ModulationType
Ieee80211MacWithRA::getControlAnswerMode(ModulationType reqMode)
{
  /**
...
void Ieee80211MacWithRA::sendUp(cMessage *msg)
{
    EV << "sending up " << msg << "\n";
...
void Ieee80211MacWithRA::removeOldTuplesFromDuplicateMap()
{
...
const MACAddress & Ieee80211MacWithRA::isInterfaceRegistered()
{
...
bool Ieee80211MacWithRA::isDuplicated(cMessage *msg)
{
...
void Ieee80211MacWithRA::promiscousFrame(cMessage *msg)
{
    if (!isDuplicated(msg)) // duplicate detection filter
 ...
 */
double Ieee80211MacWithRA::Ec(double speed, double rate, double packet_length)
{
    double PER = 0.0;
    ...
    */
    ...
    current_distance = getDistance(MobilityAccess().get()->getCurrentPosition(),apCoord);
    ...
    //previous_PER
    previous_PER = mapLastPERs.at(previous_bitrateIdx).back();//maps how to obtain PER of the  bit-rate
    //TODO
...
{
    //This method returns value = (2^val) + (2^(val-1)) + (2^(val-2)) + ... + 2^2 + 2^1
    int i = 0;
    double total = 0;
...
for each bit-rate, similar to schemes such as SampleRate [7].
 ...
 */
double Ieee80211MacWithRA::Eh(double rate, double packet_length)
{
    double PER = 0.0;
    double temp2 = 0.0;
    double temp3 = 0.0;
    ...
    {//Records loss bit-rates of  opMode(different wireless standards)
...
/**
 * Select the best bit-rate using CARS (Context-aware rate selection) algorithm.
 * context_information: speed, distance to receiver
 * alpha_weight: weight depending on speed
 * packet_length: size of packet
 * context_information will  be speed in the first version
 * gamma = 8 (for a research CARS paper)
 * POST: res = best_rate in Ieee80211DataDescriptor
 *
 */
double Ieee80211MacWithRA::cars_getrate(double context_information, double alpha_weight, int packet_length)
{
    double bitrate;
    ...
    while (idx_rate <= max_idx_rate)
    {
        ...
        throughput = bitrate / (avg_retries * pow((1 - pow(PER,transmissionLimit)),gamma));//gamma will be 8 from CARS paper
...
double Ieee80211MacWithRA::getDistance(Coord node1, Coord node2)
{
    double res = 0.0;

    res = ((node1.x-node2.x)*(node1.x-node2.x)) + ((node1.y-node2.y)*(node1.y-node2.y));// this is the distance between 2 coordinates formula
    res = sqrt(res);
